...
class FusionDM:
    def __init__(self, trunc_distance, K, tsdf_res = 256, subsample_rate = 5.0, knn = 4, marching_cubes_step_size = 3, verbose = False, write_warpfield = True):
    ...
    def compute_live_tsdf(self, depths, lws, UseAutoAlignment = False, useICP = False, outputMesh = False):
        ...
        if useICP:
            for idx in range(num_of_map):
                ...
                tsdf = np.zeros((tsdf_size,tsdf_size,tsdf_size)) + self._tdist
                tsdfw = np.zeros(tsdf.shape)
                (tsdf, tsdfw) = self.fuseDepths(depths[idx],lws[idx],tsdf,tsdfw, scale = 10 * std/self._tsdf_res, center = avg)
                if idx == 0:
                    self._tsdf = tsdf
                    self._tsdfw = tsdfw
                    self.marching_cubes()
            ...
            for idx in range(num_of_map):
                self._depthidx = idx
                print('fusing depth map ',idx)
                (tsdf, tsdfw) = self.fuseDepths(depths[idx],lws[idx],tsdf,tsdfw, scale = 12 * std/self._tsdf_res, center = avg)
            self._tsdf = tsdf
            self._tsdfw = tsdfw
    ...
    def marching_cubes(self, tsdf = None, step_size = 1):
        ...
        self._vertices, self._faces, self._normals, values = measure.marching_cubes_lewiner(self._tsdf,step_size = step_size)
        if self._verbose:
    ...
    def write_canonical_mesh(self, path, filename):
        ...
        verts, faces, normals, values = measure.marching_cubes_lewiner(self._tsdf, level = 0, step_size = 1, allow_degenerate=False)
        for v in verts:
            fpath.write('v %f %f %f\n'%(v[0],v[1],v[2]))
        for n in normals:
